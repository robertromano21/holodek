...
- EXACT REF [Image #2]: make Mortacia land inside a tight curated variant family instead of one frozen frame. Use the same 24x24 base grid language and the renderer's hardcoded drawMortaciaExact fillRect assembly, but allow 3 close variants: same iconic silhouette and pixel vocabulary, wings on the right/back with a clearer gap from her body, long hair, sword up on the left from rib height, but small changes in wing standoff, sword lift, hair drop, and leg tuck so rerolls are visibly different. Output fixed parts and design numbers that align with the ref grid (head 3-4, torso narrow 4x6-7, leg 2x10-11, etc). Palette can vary, and starter_variant/design may vary 0-2. Emulate the exact details in Image #2, not a generic approximation.
- Suzerain: match the provided knight reference through the renderer's hardcoded drawSuzerainExact grid assembly, but also use a curated 3-variant family so rerolls feel more knightly rather than just recolored. Keep the same core read: distinctive helm, left/front upright sword, right/back red cape, dark torso armor, light greaves, hands visible. Allow small variant changes in plume height, shoulder breadth, cape drop, sword height, and stance while preserving the same gallant knight silhouette. Output fixed parts (gallant_helm or equivalent, gallant_plate, armored_greaves, sword_broad, flowing_cape) + design aligning to ref proportions, and allow starter_variant/design 0-2.
