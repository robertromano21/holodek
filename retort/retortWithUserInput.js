...
console.log("personalNarrative:", personalNarrative);
 ...
 //       return actions;
    ...
    if (monstersInRoom && monstersInRoom.toLowerCase() !== 'none') {
        ...
        await $.assistant`Taking into account the player's actions, generate a list of new potential actions, including interacting with the player, NPCs, monsters or the room presently being taken by the Monsters in Room: ${monstersInRoom} using a very short sentence for each. For each action, assign a 1d20 die roll and a threshold, ranging from 1 being extremely likely and 20 being extremely unlikely to occur. Format the response as "Action (Roll: X, Threshold: Y)". Do not create any new objects. Do not create any new characters. If the player is engaging in dialogue with the monsters, formulate the actions as knowledge checks.`;
