...
console.log("personalNarrative:", personalNarrative);
  ...
  //      updatedGameConsole = updatedGameConsole.replace(/Room Name: .*/, `Room Name: ${roomName}`);
        ...
        const shouldGenerateObjects = Math.random() < 0.67;
        let objects = [];
    ...
    for (const exit of roomExitsArray.slice(1)) {
        ...
        if (shouldGeneratePuzzle) {
            ...
            $.user`This is the ${roomName}'s exits: ${roomExits}. And here are the current objects in the room: ${objectsInRoom}. Store its contents and await the next prompt.`;
            
            // Generate the puzzle description separately
            await $.assistant`Generate a detailed puzzle description in a single paragraph with no line breaks or using the word "you" that fits the room description for ${roomName} but do not reveal that the player is looking at a puzzle and do not reveal the solution.`;
            ...
            await $.assistant`Generate a solution for the puzzle in a single paragraph with no line breaks or using the word "you" in ${roomName}. The puzzle should be challenging but solvable, with clear logic and engaging elements and the solution should be clear, logical, and involve potential player actions or items they might carry.`;
...
function dropMonsterItemsToRoom(targetMonster) {
    ...
    // For each equipped item of the monster
    for (const slot in targetMonster.equipped) {
        const itemName = targetMonster.equipped[slot];
        if (itemName && itemName !== 'None') {
            // Find the item properties from monstersEquippedProperties
            ...
            if (itemIndex !== -1) {
                ...
                // Add itemName to Objects in Room if not already present
                if (!objectsInRoom.includes(itemName)) {
                    objectsInRoom.push(itemName);
                }

                // Add itemProperties to Objects in Room Properties if not already present
                if (!objectsInRoomProperties.some(item => item.name === itemName)) {
                    objectsInRoomProperties.push(itemProperties);
                }

                // Remove the item from monstersEquippedProperties
                monstersEquippedProperties.splice(itemIndex, 1);
            } else {
                // If item not found, log an error and skip
    ...
    // Update the updatedGameConsole with the new Objects in Room and Objects in Room Properties
    const updatedObjectsInRoomString = objectsInRoom.length > 0 ? objectsInRoom.join(', ') : 'None';

    // Convert objectsInRoomProperties to a formatted string
    ...
    // Update the Monsters Equipped Properties in updatedGameConsole
    ...
    const newEquippedLine = `${Object.entries(targetMonster.equipped).map(([slot, item]) => `${slot}: ${item}`).join(', ')}`;

    // Use a regex to replace the 'Equipped' line for the monster
...
//    console.log(diceRoll);
 ...
 //   $.user``;  //  $.user`Current game console: ${updatedGameConsole}`;;
   ...
   $.user`This is the ${roomName}'s description: "${roomDescription}" Store its contents and await the next prompt.`;
   $.user`This is the ${roomName}'s current puzzle: "${puzzleDescription}" Store its contents and await the next prompt.`;
