...
The player character and every NPC listed under "NPCs in Party" have just regained consciousness moments ago in the Ruined Temple of Tartarus, except Mortacia. Each character remembers his or her individual identity, but none of the non-divine characters recognize who the others are nor do they understand where they are or how they got here, as their recent memories may be slightly fragmented, contradictory, emotional, and personal. 
...
- EXACT REF [Image #2]: make Mortacia land inside a curated 6-variant family instead of one frozen frame. Variants 0-2 may stay in the renderer's hardcoded drawMortaciaExact fillRect language, but variants 3-5 should branch into the same prefab pipeline as other sprites while keeping her core identity. Mortacia ALWAYS has wings. Keep wings on the right/back with a clearer gap from her body, long hair, weapon on the left from rib height, but allow stronger reroll differences: dress silhouettes, corset silhouettes, sword/spear/mace variants, idle/striding/defensive feminine poses, different wing standoff, hair drop, and leg tuck. Output fixed parts/design numbers that align with the 24x24 ref grid. Palette can vary, and starter_variant/design may vary 0-5. Emulate the exact details in Image #2 for the exact variants, not a generic approximation.
- Suzerain: match the provided knight reference through the renderer's hardcoded drawSuzerainExact grid assembly and keep him visually locked to that iconic read. Distinctive helm, left/front upright weapon, right/back red cape, dark torso armor, light greaves, hands visible. Minor color/detail variation is acceptable, but do NOT invent alternate silhouettes, alternate weapons, or alternate poses for Suzerain. Prefer the exact gallant knight loadout and proportions.
...
RULES:
- Mortacia (tall goddess with grey dragon wings): ALWAYS choose head:"human_hair" (reduce head by 20% +1 wider/longer for visible face + more defined feminine hair + simple single-line cap as thin rect on top, NO skull_crest), accessory:"skeletal_wings" (or bone_wings), and ALWAYS keep wings visible. Use torso from "cinched_corset" / "bone_corset" / "dress_robe" / "tapered_robe", legs from "long_striders" / "flowing_skirt" / "striding_boots", and weapon from "sword_broad" primarily but also "spear" or "mace" for some starter variants while keeping the core design. Use design for tall (leg_height:12-13), pulpy slender body (torso_width/leg_thickness ~2-3 for thinner, some form not ultra-thin), head_size/head_width reduced 20% +1 (smaller but visible face), wing_bone_count:4-5, wing_length:2. Give longer more defined feminine hair. Poses can include "attack_slash_female", "idle_stand_female", "striding_elegant_female", or "defend_dodge_female", but weapon grip/hand MUST still start at ribs and weapon must stay on the LEFT while wings stay on the RIGHT with a slightly clearer gap from her back. Grey dragon bones should be thick + fanned + large span for pulpy wings.
...
- Suzerain (gallant knight with helm, cape and armor per ref image [Image #1]): produce the EXACT layout via the renderer's drawSuzerainExact read whenever possible (distinctive light helm w/ dark cross/visor, skin hands at each side with left gripping upright sword, red cape right/back, dark armor w/ belt, light greaves w/ dark bands, dark feet). Keep him on head:"gallant_helm", torso:"gallant_plate", legs:"armored_greaves", accessory:"flowing_cape", weapon:"sword_broad". Minor design-number nudges and color variation are fine, but do not broaden him into other knight families.
...
- WEAPON VARIETY IS MANDATORY on every generation/reroll for generic characters: choose a DIFFERENT weapon each time from the full list. For Mortacia specifically prefer sword_broad most often, but allow some starter-family variants to use spear or mace while preserving her core winged goddess silhouette. Use widely varying weapon_length + blade_size + pose hold so the weapon silhouette itself changes the whole figure look.
- For rerolls and variety: pick different catalog types AND different "pose" from the dozens listed when possible for generic characters. Mortacia may vary within her curated starter family. Suzerain should stay visually locked to his exact knight identity and should not broaden into alternate poses or weapon silhouettes.
