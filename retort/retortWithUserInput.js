...
console.log("personalNarrative:", personalNarrative);
 ...
 //       return actions;
    ...
    if (npcsInParty && npcsInParty.toLowerCase() !== 'none') {
        ...
        await $.assistant`Taking into account the player's actions, generate a list of new potential actions, including interacting with the player, NPCs, monsters or the room presently being taken by the NPCs in Party: ${npcsInParty} using a very short sentence for each. For each action, assign a 1d20 die roll and a threshold, ranging from 1 being extremely likely and 20 being extremely unlikely to occur. Format the response as "Action (Roll: X, Threshold: Y)". Do not create any new objects. Do not create any new characters.`;
...
//    console.log(diceRoll);
 ...
 //   $.user``;  //  $.user`Current game console: ${updatedGameConsole}`;;
    ...
    $.user`This is the current room's game console: ${updatedGameConsole}`;
    ...
    $.user`These are the current actions being taken by NPCs and monsters in the room: ${outcomes}. If a roll equals or exceeds the roll threshold, then the action was successful and if it less than the roll threshold, it fails. Seamlessly incorporate the results of actions into the story's narrative.`;
